...
from rag_chain import SearchMethod, LangChainRAG, RAGConfig
...
class AppSettings:
    ...
    MODEL_NAME = os.getenv("VLLM_MODEL_NAME", "gemma-3-1b-it")
...
async def lifespan(app: FastAPI):
    ...
    app_state["rag_chain"] = LangChainRAG(
        config=RAGConfig(
...
async def chat(query_input: QueryInput, request: Request):
    """Chat endpoint using LangChain RAG"""
    rag = app_state.get("rag_chain")
    if rag is None:
    ...
    if result["success"]:
        insert_application_logs(
            query_input.session_id or str(uuid.uuid4()),
